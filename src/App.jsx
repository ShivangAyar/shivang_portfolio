...
function MechanicalCore({ isHovered, scrollY, isMobile }) {
  ...
  // Logic: Assemble ONLY at top. Desktop needs hover. Mobile auto-snaps at top.
      ...
      for (let y = 0; y < gridSize; y++) {
        for (let z = 0; z < gridSize; z++) {
          ...
          const randomPos = new THREE.Vector3(
            (Math.random() - 0.5) * (isMobile ? 35 : 65),
            (Math.random() - 0.5) * (isMobile ? 35 : 65),
            (Math.random() - 0.5) * 25
          );
...
function FloatingHUDText({ text, offset, isAssembled, p }) {
  ...
  useFrame((state) => {
    const t = state.clock.getElapsedTime();
    if (ref.current) {
        const driftX = Math.sin(t * 0.3 + p) * (isAssembled ? 0.5 : 22);
        const driftY = Math.cos(t * 0.3 + p) * (isAssembled ? 0.5 : 18);
        ref.current.position.lerp(new THREE.Vector3(offset[0] + driftX, offset[1] + driftY, isAssembled ? 0 : -15), 0.03);
        ref.current.lookAt(state.camera.position);
    }
  });
  return (
    <Text ref={ref} fontSize={0.6} font="https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuLyfMZhrib2Bg-4.ttf" color="#00E5FF" emissive="#00E5FF" emissiveIntensity={isAssembled ? 5 : 0.1} transparent opacity={isAssembled ? 1 : 0.15} />
  );
}
...
const CompetencyCard = ({ title, icon, skills, isMobile }) => {
  ...
  return (
    <div 
      ...
      className="bg-[#050507]/60 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/5 hover:border-[#00E5FF]/40 transition-all duration-700 shadow-2xl group flex flex-col h-[420px]"
    >
      ...
      <div className="relative z-10 w-full flex flex-col px-6 md:px-16">
        ...
        <footer id="connect" className="w-full py-60 flex items-center justify-center">
          <div className="max-w-6xl w-full bg-[#020203] border-2 border-[#00E5FF]/40 p-16 md:p-24 rounded-[5rem] shadow-[0_0_120px_rgba(0,229,255,0.15)] text-center relative overflow-hidden group">
            ...
            <div className="absolute top-0 right-0 p-12 text-[8px] font-mono text-gray-700 text-right uppercase hidden md:block leading-loose opacity-40">Latency: 12ms<br/>Encryption: AES-256</div>
