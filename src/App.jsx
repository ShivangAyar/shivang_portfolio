...
function MechanicalCore({ isHovered, isMobile, scrollY }) {
  ...
  // Assembly logic strictly separated
  const isAssembled = isMobile 
    ? (scrollY < 150) // Mobile: Auto-assemble at top
    : (isHovered && scrollY < 150); // Laptop: Hover-assemble at top
  ...
  const words = useMemo(() => [
    ...
    { text: "ARCHITECTURE", offset: [6.8, 0, 0], p: 0.5 },
    { text: "MERN STACK", offset: [-6.8, 0, 0], p: 0.8 },
    { text: "FULL-STACK", offset: [0, -4.8, 0], p: 1.2 }
  ], []);

  useFrame((state, delta) => {
    const t = state.clock.getElapsedTime();
    groupRef.current.rotation.y += delta * (isAssembled ? 0.2 : 0.02); // Very slow drift
    ...
    cubeData.forEach((data, i) => {
      ...
      currentPositions[i].lerp(targetP, isAssembled ? 0.1 : 0.008); // Slow dispersal
      currentRotations[i].slerp(targetR, isAssembled ? 0.1 : 0.005);
...
function FloatingHUDText({ text, isAssembled, offset, phase, isMobile }) {
  ...
  useFrame((state) => {
    ...
    if (ref.current) {
      ...
      const driftX = Math.sin(t * 0.2 + phase) * (isAssembled ? 0.6 : 18);
      const driftY = Math.cos(t * 0.2 + phase) * (isAssembled ? 0.6 : 15);
      ref.current.position.lerp(new THREE.Vector3(offset[0] + driftX, offset[1] + driftY, isAssembled ? 0 : -10), 0.015);
      ref.current.lookAt(state.camera.position);
    }
  });
  return (
    <Text ref={ref} fontSize={isMobile ? 0.45 : 0.6} font="https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuLyfMZhrib2Bg-4.ttf" color="#00E5FF" emissive="#00E5FF" emissiveIntensity={isAssembled ? 5 : 0.1} transparent opacity={isAssembled ? 1 : 0.2}>
      {text}
    </Text>
  );
}
...
const CompetencyCard = ({ title, icon, skills, isMobile }) => {
  ...
  // Mobile stays reactive (isInView), Laptop is reactive on hover
  ...
  return (
    <motion.div 
      ...
      whileHover={!isMobile ? { y: -10, borderColor: "rgba(0, 229, 255, 0.4)" } : {}}
      className="bg-[#0A0A12]/40 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/5 transition-all duration-500 shadow-2xl group relative"
    >
      <div className="absolute top-0 right-0 p-8 opacity-10 text-3xl">{icon}</div>
      <h3 className="text-xs font-black text-white mb-10 tracking-[0.5em] uppercase flex items-center gap-4">
        <span className={`w-2 h-2 rounded-full transition-all duration-500 ${active ? 'bg-[#00E5FF] shadow-[0_0_15px_#00E5FF]' : 'bg-gray-800'}`} /> {title}
      </h3>
      <div className="space-y-10">
        {skills.map((s, i) => (
          <div key={i} className="relative">
            <div className="flex justify-between text-[11px] font-bold text-gray-500 uppercase mb-3 tracking-widest">
              <span className={active ? 'text-gray-300' : ''}>{s.n}</span><span className={active ? 'text-[#00E5FF]' : ''}>{s.v}</span>
            </div>
            <div className="flex gap-2 h-[6px]">
              {[...Array(12)].map((_, idx) => (
                <motion.div
                  key={idx}
                  initial={{ backgroundColor: "#14141B" }}
                  animate={active ? { backgroundColor: idx * (100/12) < parseInt(s.v) ? "#00E5FF" : "#14141B" } : {}}
                  transition={{ delay: idx * 0.04 + i * 0.1, duration: 0.3 }}
                  className="flex-1 rounded-sm"
                  style={{ boxShadow: active && idx * (100/12) < parseInt(s.v) ? '0 0 10px rgba(0, 229, 255, 0.3)' : 'none' }}
                />
              ))}
            </div>
          </div>
  ...
  return (
    <div className="bg-[#010102] text-gray-200 antialiased selection:bg-[#00E5FF] selection:text-black font-sans scroll-smooth relative min-h-screen overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <Canvas dpr={[1, 2]}>
          <PerspectiveCamera makeDefault position={[0, 0, isMobile ? 26 : 18]} fov={isMobile ? 70 : 45} />
      ...
      <div className="relative z-30 w-full flex flex-col">
        ...
        {/* NAVBAR */}
        <nav className="fixed top-0 w-full z-[200] bg-[#010102]/60 backdrop-blur-xl border-b border-white/5 h-20 md:h-24 flex items-center justify-between px-10 md:px-20">
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[190] bg-[#010102] flex flex-col items-center justify-center gap-10 backdrop-blur-3xl">
        ...
        {/* HERO */}
        <section className="max-w-7xl mx-auto px-10 min-h-screen flex items-center relative w-full pt-20" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1 }} className="flex flex-col items-start max-w-4xl z-40">
            <div className="mb-6 px-4 py-1.5 border border-[#00E5FF]/20 bg-[#00E5FF]/5 text-[#00E5FF] text-[10px] font-black tracking-[0.5em] uppercase">Architecting Logic</div>
            ...
            <p className="text-lg md:text-2xl text-gray-400 mt-6 mb-12 font-light max-w-2xl border-l-2 border-[#FF8C00] pl-8 leading-relaxed">Designing high-performance systems and full-stack digital architectures.</p>
            <div className="flex gap-6 pointer-events-auto">
              <a href="#projects" className="bg-white text-black px-10 py-5 text-[10px] font-black tracking-[0.3em] uppercase hover:bg-[#00E5FF] hover:text-white transition-all">Execute Builds</a>
        ...
        <section id="about" className="max-w-7xl mx-auto px-10 py-40 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
            <div className="lg:col-span-5">
              ...
              <p className="text-gray-400 text-xl font-light leading-relaxed mb-12">Born in Zambia, based in Ottawa. My approach is purely objective: Build, Optimize, Master.</p>
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-white/5 p-10 rounded-[2.5rem] border border-white/5"><h3 className="text-5xl font-black text-white">3+</h3><p className="text-[10px] uppercase tracking-widest text-gray-500 mt-2 font-black">Years</p></div>
                <div className="bg-white/5 p-10 rounded-[2.5rem] border border-white/5"><h3 className="text-5xl font-black text-white">10+</h3><p className="text-[10px] uppercase tracking-widest text-gray-500 mt-2 font-black">Builds</p></div>
              </div>
            </div>
            <div className="lg:col-span-7 space-y-8">
              {[ {y:"2024 - PRESENT", t:"Algonquin College", d:"Advanced Diploma in Computer Programming. Enterprise focus."},
                 {y:"2021 - 2023", t:"Fraser International College", d:"Computer Science Pathway. Foundational expertise in algorithmic logic."}
              ].map((item, i) => (
                <div key={i} className="bg-white/5 p-10 rounded-[3rem] border border-white/5 relative overflow-hidden group">
        ...
        <section id="projects" className="max-w-7xl mx-auto px-10 py-40">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[
              { title: "E-Commerce Microservices", d: "Scaleable backend utilizing Docker, Stripe API, and JWT auth.", t: ["Node.js", "Docker", "Stripe"] },
              { title: "Movie Watchlist App", d: "Full-stack media tracker via RESTful APIs and NoSQL architecture.", t: ["MongoDB", "Express", "Node"] },
              { title: "Real-Time Collab", d: "Live-syncing environment using WebSockets for real-time editing.", t: ["Socket.io", "Next.js", "Redis"] },
              { title: "Voice AI Chatbot", d: "Emotion-aware chatbot integrating OpenAI and Voice APIs.", t: ["React", "OpenAI", "WebRTC"] },
              { title: "DevOps Dashboard", d: "Command center for GitHub Actions and AWS deployment metrics.", t: ["AWS", "Python", "GitHub"] },
              { title: "AI SaaS Generator", d: "SaaS wrapping OpenAI featuring user credits and async generation.", t: ["Tailwind", "React", "DALL-E"] }
            ].map((p, i) => (
              <motion.div key={i} whileHover={!isMobile ? { y: -15 } : {}} className="bg-[#0A0A12]/40 backdrop-blur-xl p-12 rounded-[3.5rem] border border-white/5 relative overflow-hidden h-[480px] flex flex-col group cursor-pointer shadow-2xl">
        ...
        <footer id="contact" className="w-full py-60 flex items-center justify-center px-10">
          <div className="max-w-6xl w-full bg-[#030305] border border-white/10 p-16 md:p-24 rounded-[5rem] shadow-[0_0_100px_rgba(0,0,0,1)] text-center relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 relative z-[210]">
              <a href="https://github.com" target="_blank" className="bg-white/5 border border-white/10 px-10 py-6 rounded-3xl text-white font-black tracking-widest uppercase hover:bg-white hover:text-black transition-all">GitHub</a>
              <a href="https://linkedin.com" target="_blank" className="bg-white/5 border border-white/10 px-10 py-6 rounded-3xl text-white font-black tracking-widest uppercase hover:bg-[#00E5FF] hover:text-black transition-all">LinkedIn</a>
