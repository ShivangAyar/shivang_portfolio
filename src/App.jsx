...
function CentralReactiveShape() {
  ...
  useFrame((state, delta) => {
    ...
    meshRef.current.position.x = THREE.MathUtils.lerp(meshRef.current.position.x, state.pointer.x * 1, 0.05);
    meshRef.current.position.y = THREE.MathUtils.lerp(meshRef.current.position.y, state.pointer.y * 1, 0.05);
  });
  ...
  return (
    <Float speed={3} rotationIntensity={0.5} floatIntensity={1}>
      ...
      <mesh scale={1.1} rotation={[Math.PI/4, 0, 0]}>
        <icosahedronGeometry args={[1, 1]} />
...
const NavLink = ({ href, children }) => (
  <a href={href} className="group relative py-2 text-[10px] font-bold tracking-[0.3em] text-gray-500 uppercase transition-colors hover:text-white">
    {children}
    <span className="absolute bottom-0 left-0 h-[2px] w-full origin-left scale-x-0 bg-gradient-to-r from-[#00E5FF] to-[#FF3366] transition-transform duration-500 ease-out group-hover:scale-x-100 shadow-[0_0_10px_#00E5FF]" />
  </a>
);
    ...
    <div className="bg-[#020204] text-gray-200 antialiased selection:bg-[#00E5FF] selection:text-black font-sans scroll-smooth relative min-h-screen overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-auto">
        <Canvas camera={{ position: [0, 0, 15], fov: 45 }} shadows>
          ...
          {/* Background Words with Correct Viewport Placement */}
      ...
      <div className="relative z-30 w-full flex flex-col">
        ...
        <nav className="fixed top-0 w-full z-50 bg-[#020204]/60 backdrop-blur-3xl border-b border-white/5 h-20 md:h-24 flex items-center justify-between px-8">
          <div className="text-xl md:text-2xl font-black tracking-tighter text-white cursor-pointer group" onClick={() => window.scrollTo(0,0)}>
            SHIVANG<span className="text-[#00E5FF] group-hover:text-[#FF3366] transition-colors duration-500">.</span>
          </div>
        ...
        <section className="max-w-7xl mx-auto px-6 min-h-screen flex items-center pointer-events-none relative w-full pt-20">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.2, ease: "easeOut" }} className="flex flex-col items-start pointer-events-auto max-w-4xl">
            ...
            <h1 className="text-6xl sm:text-8xl md:text-[9.5rem] font-black text-white mb-4 tracking-tighter leading-[0.85]">Shivang <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00E5FF] via-white to-[#FF3366] animate-gradient-x">Ayar.</span></h1>
            <p className="text-lg md:text-2xl text-gray-400 mt-8 mb-12 leading-relaxed font-light max-w-2xl border-l-2 border-[#FF3366] pl-6">Engineering scalable digital ecosystems with precision backends and immersive frontend architectures.</p>
        ...
        {/* ABOUT & JOURNEY */}
        <section id="about" className="max-w-7xl mx-auto px-6 py-40 pointer-events-none w-full relative">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 pointer-events-auto items-center">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="lg:col-span-5">
              ...
              <p className="text-gray-400 mb-8 text-xl leading-relaxed font-light">Born and raised in Zambia, I brought my drive across the globe to engineer software in Canada. I build with the same discipline I apply to my 6-day gym split—constant progression and mastery.</p>
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="bg-[#0A0A12]/80 border border-white/5 p-8 rounded-3xl text-center shadow-2xl hover:border-[#FF3366]/40 transition-colors group">
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:col-span-7 space-y-12 border-l-2 border-[#00E5FF]/20 ml-4 relative">
              {[ {i:"🎓", y:"2024 - Present", t:"Algonquin College | Ottawa", p:"Advanced Diploma in Computer Programming & Analysis. Specialized in enterprise architecture.", c:"#00E5FF"},
                 {i:"💻", y:"2021 - 2023", t:"Fraser International College | BC", p:"Computer Science Pathway. Foundational expertise in algorithm design.", c:"#A855F7"}
              ].map((step, idx) => (
        ...
        {/* CORE COMPETENCIES */}
        <section id="skills" className="max-w-7xl mx-auto px-6 py-32 pointer-events-none w-full relative">
        ...
        {/* PROJECTS SECTION */}
        <section id="projects" className="max-w-7xl mx-auto px-6 py-40 pointer-events-none w-full relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 pointer-events-auto [perspective:2000px]">
                {projects.map((p, i) => (
                    <motion.div 
                        key={i} 
        ...
        {/* OFFLINE PROTOCOL */}
        <section className="max-w-7xl mx-auto px-6 py-40 flex flex-col items-center w-full relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10 w-full pointer-events-auto">
              {[ {i:"🏋️‍♂️",t:"Iron & Discipline",d:"6-day splits focusing on heavy compound progression and physiological optimization."},
                 {i:"🎮",t:"Gaming & Logic",d:"Strategic exploration, hardware tuning, and competitive mechanics optimization."},
                 {i:"🌍",t:"Hiking & Travel",d:"Exploration of unfamiliar terrain to maintain equilibrium outside the digital terminal."}
              ].map((h,x)=>(
                <div key={x} className="bg-[#0A0A12]/40 backdrop-blur-3xl border border-white/5 p-12 rounded-[2.5rem] text-center hover:border-white/20 transition-all duration-500 shadow-2xl group">
                  <div className="text-7xl mb-8 grayscale opacity-20 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700 [transform:scale(1)] group-hover:scale-110">{h.i}</div>
        ...
        {/* TERMINAL CONTACT (The Floating Card) */}
