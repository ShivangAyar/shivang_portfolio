  ...
  return (
    <group ref={groupRef} position={[isMobile ? 0 : 3.8, 0, 0]}>
      ...
      {["SHIVANG", "ARCHITECTURE", "MERN STACK", "FULL-STACK"].map((txt, i) => (
        <HUDLabel key={txt} text={txt} offset={[[0, 5, 0], [7.5, 0, 0], [-7.5, -2, 0], [0, -5, 0]][i]} isAssembled={isAssembled} p={i * 0.4} />
      ))}
    </group>
  );
}
...
function HUDLabel({ text, offset, isAssembled, p }) {
  ...
  useFrame((state) => {
    const t = state.clock.getElapsedTime();
    if (ref.current) {
        const dX = Math.sin(t * 0.3 + p) * (isAssembled ? 0.5 : 20);
        const dY = Math.cos(t * 0.3 + p) * (isAssembled ? 0.5 : 15);
        ref.current.position.lerp(new THREE.Vector3(offset[0] + dX, offset[1] + dY, isAssembled ? 0 : -10), 0.03);
        ref.current.lookAt(state.camera.position);
    }
  });
  return (
    <Text ref={ref} fontSize={0.65} font="https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuLyfMZhrib2Bg-4.ttf" color="#00E5FF" emissive="#00E5FF" emissiveIntensity={isAssembled ? 5 : 0.15} transparent opacity={isAssembled ? 1 : 0.25}>{text}</Text>
  );
}

// --- REVERSED REACTIVE SLIDER COMPONENT ---
const CompetencyCard = ({ title, icon, skills, isMobile }) => {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="bg-[#0A0A12]/40 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/5 hover:border-[#00E5FF]/40 transition-all duration-700 shadow-2xl group flex flex-col h-[420px]">
      ...
      <div className="space-y-8 mt-auto">
        {skills.map((s, i) => (
          <div key={i}>
            ...
            <div className="flex gap-1.5 h-[4px] overflow-hidden bg-white/5 rounded-full">
              {[...Array(10)].map((_, idx) => (
                <motion.div
                  key={idx}
                  initial={{ backgroundColor: "#1A1A24" }}
                  animate={active ? { backgroundColor: idx * 10 < parseInt(s.v) ? "#00E5FF" : "#1A1A24" } : {}}
                  transition={{ delay: idx * 0.05 + i * 0.1 }}
                  className="flex-1 rounded-sm"
                  style={{ backgroundImage: active && idx * 10 < parseInt(s.v) ? 'linear-gradient(to right, #00E5FF, #FF8C00)' : 'none', opacity: active ? 1 : 0.3 }}
                />
              ))}
            </div>
          </div>
...
export default function App() {
  ...
  return (
    <div className="bg-[#010102] text-gray-200 antialiased selection:bg-[#00E5FF] font-sans scroll-smooth relative min-h-screen overflow-x-hidden">
      ...
      {/* 2D LIGHT TRACKER */}
      {!isMobile && (
        <div className="pointer-events-none fixed inset-0 z-20" style={{ background: `radial-gradient(850px circle at ${mousePos.x}px ${mousePos.y}px, rgba(0, 229, 255, 0.045), transparent 85%)` }} />
      )}
      ...
      <div className="relative z-30 w-full flex flex-col px-6 md:px-16">
        ...
        {/* NAVBAR */}
        <nav className="fixed top-0 left-0 w-full z-[100] bg-[#010102]/60 backdrop-blur-xl border-b border-white/5 h-20 md:h-24 flex items-center justify-between px-8 md:px-20">
          ...
          <div className="hidden md:flex gap-12 relative">
            {["Journey", "Stacks", "Builds", "Offline", "Connect"].map((tab) => (
              <a key={tab} href={`#${tab.toLowerCase()}`} onClick={() => setActiveTab(tab)} className="relative py-2 text-[10px] font-black tracking-[0.3em] uppercase text-gray-500 hover:text-white transition-colors">
                {tab}
                {activeTab === tab && <motion.div layoutId="nav-slider" className="absolute bottom-0 left-0 h-[2px] w-full bg-gradient-to-r from-[#00E5FF] to-[#FF8C00] shadow-[0_0_15px_#00E5FF]" />}
              </a>
            ))}
          </div>
          <button onClick={() => setIsMenuOpen(true)} className="md:hidden text-[#00E5FF] p-2">
        ...
        <section id="journey" className="max-w-7xl mx-auto px-6 py-40 w-full relative">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            <div className="lg:col-span-7 space-y-12 border-l-2 border-[#00E5FF]/20 ml-4 relative">
              {[
                { y: "2024 - PRESENT", t: "Algonquin College | Ottawa", d: "Advanced Diploma in Computer Programming. Focused on Enterprise Java, MERN stacks, and high-scale cloud-native logic using Docker/AWS." },
                { y: "2021 - 2023", t: "Fraser International College | BC", d: "Computer Science Pathway. Foundational deep-dive into Big O efficiency, data structure optimization, and OOP logic." }
              ].map((item, idx) => (
        ...
        <section id="builds" className="max-w-7xl mx-auto px-6 py-40">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[
              ...
              { title: "Movie Watchlist App", d: "Engineered a full-stack media tracking application using the MERN stack. Designed a robust RESTful API to manage user watchlists and utilized MongoDB's NoSQL architecture for high-speed data retrieval.", t: ["MongoDB", "Express", "Node"] },
              { title: "Real-Time Collaboration", d: "Developed a synchronized workspace using Socket.io and Next.js. Implemented real-time document editing and instant messaging through WebSockets, backed by a high-performance Redis caching layer.", t: ["Socket.io", "Next.js", "Redis"] },
              { title: "Voice AI Chatbot", d: "Built an advanced AI chatbot with integrated emotion recognition logic. Leveraged OpenAI's GPT models and WebRTC for low-latency voice communication, providing a natural and immersive user experience.", t: ["React", "OpenAI", "WebRTC"] },
              { title: "DevOps Metrics Hub", d: "Constructed a monitoring platform for GitHub Actions and AWS infrastructure. Developed an automated ETL pipeline using Python to aggregate and visualize deployment metrics on a custom dashboard.", t: ["AWS", "Python", "GitHub"] },
              { title: "AI SaaS Generator", d: "Launched a SaaS platform for asynchronous AI image generation. Built with React and Tailwind CSS, featuring a credit-based user system and multi-modal API integration with DALL-E models.", t: ["Tailwind", "React", "DALL-E"] }
            ].map((p, i) => (
              <motion.div key={i} whileHover={!isMobile ? { y: -15 } : {}} className="bg-[#0A0A12]/40 backdrop-blur-xl p-12 rounded-[3.5rem] border border-white/5 relative overflow-hidden h-[540px] flex flex-col group cursor-pointer shadow-2xl">
        ...
        <section id="offline" className="max-w-7xl mx-auto px-10 py-40 w-full text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {[ {i:"🏋️‍♂️",t:"Discipline",d:"6-day compound split focus, applying progressive overload to mastery."},
                 ...
                 {i:"🌍",t:"Equilibrium",d:"Hiking and exploration to reset the digital buffer."}
              ].map((h,x)=>(
                <motion.div key={x} initial={{scale:0.95, opacity: 0}} whileInView={{scale:1, opacity: 1}} transition={{duration: 0.5}} className="bg-white/5 p-12 rounded-[3.5rem] border border-white/5 hover:border-[#FF8C00]/40 transition-all duration-500 group shadow-2xl">
                  <div className="text-7xl mb-10 opacity-100 group-hover:scale-110 transition-transform">{h.i}</div>
        ...
        <footer id="connect" className="w-full py-40 flex items-center justify-center px-6">
          <div className="max-w-6xl w-full bg-[#020203] border-2 border-[#00E5FF]/40 p-12 md:p-24 rounded-[4rem] shadow-[0_0_120px_rgba(0,229,255,0.15)] text-center relative overflow-hidden group">
            ...
            <div className="flex items-center justify-center gap-4 mb-20 bg-white/5 w-max mx-auto px-10 py-4 rounded-full border border-white/10">
              ...
              <span className="text-[10px] font-black tracking-[0.5em] uppercase text-green-400">Secure Line</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 relative z-50">
              {["GitHub", "LinkedIn", "Email"].map((l, i) => (
                <a key={l} href={i === 2 ? "mailto:ayarshivang27@gmail.com" : "#"} target="_blank" className="bg-white/5 border border-white/10 px-10 py-6 rounded-3xl text-white font-black tracking-widest uppercase hover:bg-white hover:text-black transition-all">{l}</a>
              ))}
            </div>
            <p className="mt-32 text-[10px] font-black tracking-[1.5em] text-gray-800 uppercase">© 2026 SHIVANG AYAR. CRAFTED WITH INTENT.</p>
          </div>
        </footer>
