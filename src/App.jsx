...
function MechanicalCore({ isHovered, scrollY, isMobile }) {
  ...
  // Mobile logic: Assemble only at top (scrollY < 120). 
  // Laptop logic: Assemble only at top + hover.
  ...
  useFrame((state, delta) => {
    ...
    groupRef.current.rotation.y += delta * (isAssembled ? 0.3 : 0.03); // Slower drift when dispersed
    ...
    cubeData.forEach((data, i) => {
      ...
      currentPositions[i].lerp(targetP, isAssembled ? 0.12 : 0.01); 
      currentRotations[i].slerp(targetR, isAssembled ? 0.12 : 0.01);
    ...
    <group ref={groupRef} position={[isMobile ? 0 : 3.8, 0, 0]}>
      <instancedMesh ref={meshRef} args={[null, null, count]}>
        ...
        <meshStandardMaterial 
          color={isAssembled ? "#00E5FF" : "#0A2424"} 
          metalness={1} 
          roughness={0.1} 
      ...
      {["SHIVANG", "ARCHITECTURE", "MERN STACK", "FULL-STACK"].map((txt, i) => (
        <HUDLabel key={txt} text={txt} offset={[[0, 5.2, 0], [7.8, 0, 0], [-7.8, -2, 0], [0, -5.2, 0]][i]} isAssembled={isAssembled} p={i * 0.4} />
      ))}
    </group>
  );
}
...
function HUDLabel({ text, offset, isAssembled, p }) {
  ...
  useFrame((state) => {
    ...
    if (ref.current) {
        ...
        ref.current.position.lerp(new THREE.Vector3(offset[0] + dX, offset[1] + dY, isAssembled ? 0 : -12), 0.03);
        ref.current.lookAt(state.camera.position);
    }
  });
  return (
    <Text ref={ref} fontSize={0.65} font="https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuLyfMZhrib2Bg-4.ttf" color="#00E5FF" emissive="#00E5FF" emissiveIntensity={isAssembled ? 5 : 0.1} transparent opacity={isAssembled ? 1 : 0.2}>{text}</Text>
  );
}
...
const CompetencyCard = ({ title, icon, skills, isMobile }) => {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className="bg-[#0A0A12]/60 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/5 hover:border-[#00E5FF]/40 transition-all duration-700 shadow-2xl group flex flex-col h-[420px]">
    ...
    <div className="bg-[#010102] text-gray-200 antialiased font-sans scroll-smooth relative min-h-screen overflow-x-hidden">
      ...
      <div className="relative z-30 w-full flex flex-col px-6 md:px-16">
        ...
        <section id="journey" className="max-w-7xl mx-auto px-6 py-40 w-full relative">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            <div className="lg:col-span-7 space-y-12 border-l-2 border-[#00E5FF]/20 ml-4 relative">
              {[{y:"2024 - PRESENT", t:"Algonquin College | Ottawa", d:"Advanced Diploma in Computer Programming. Deep specialization in Enterprise Java, MERN stacks, and high-scale cloud-native logic."},
                {y:"2021 - 2023", t:"Fraser International College | BC", d:"Computer Science Pathway. Foundational deep-dive into Big O efficiency, data structure optimization, and OOP principles."}].map((item, idx) => (
        ...
        <section id="builds" className="max-w-7xl mx-auto px-6 py-40">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[
              { title: "E-Commerce Services", d: "Architected a scalable microservices ecosystem using Node.js and Docker. Integrated Stripe API for secure payments and implemented JWT authentication.", t: ["Node.js", "Docker", "Stripe"] },
        ...
        <footer id="connect" className="w-full py-40 flex items-center justify-center px-6">
          <div className="max-w-6xl w-full bg-[#020203] border-2 border-[#00E5FF]/40 p-12 md:p-24 rounded-[4rem] shadow-[0_0_120px_rgba(0,229,255,0.15)] text-center relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 relative z-50">
              ...
              <a href="https://www.linkedin.com/in/shivangayar/" target="_blank" className="bg-white/5 border border-white/10 px-10 py-6 rounded-3xl text-white font-black tracking-widest uppercase hover:bg-[#00E5FF] hover:text-black transition-all">LinkedIn</a>
