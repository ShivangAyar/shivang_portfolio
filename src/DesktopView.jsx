...
const projects = [
  { title: "E-Commerce Microservices", d: "Architected a scalable microservices ecosystem using Node.js and Docker. Integrated Stripe API and implemented JWT-based authentication.", t: ["Node.js", "Docker", "Stripe"] },
  { title: "Movie Watchlist App", d: "Engineered a full-stack media tracking application using the MERN stack. Designed a robust RESTful API and utilized MongoDB NoSQL architecture.", t: ["MongoDB", "Express", "Node"] },
  { title: "Real-Time Collaboration", d: "Developed a synchronized workspace using Socket.io and Next.js with performance Redis caching.", t: ["Socket.io", "Next.js", "Redis"] },
  { title: "Voice AI Chatbot", d: "Built an advanced AI chatbot with integrated emotion recognition. Leveraged OpenAI models and WebRTC for low-latency communication.", t: ["React", "OpenAI", "WebRTC"] },
  { title: "DevOps Metrics Hub", d: "Constructed a monitoring platform for GitHub Actions using Python ETL pipelines to visualize deployment metrics.", t: ["AWS", "Python", "GitHub"] },
  { title: "AI SaaS Generator", d: "Launched a SaaS platform for asynchronous AI image generation featuring a credit-based user system.", t: ["Tailwind", "React", "DALL-E"] }
];
...
function Core({ isHovered, scrollY }) {
  ...
  useFrame((state, delta) => {
    ...
    cubeData.forEach((data, i) => {
      ...
      cP[i].lerp(tP, isAssembled ? 0.12 : 0.015);
      cR[i].slerp(tR, isAssembled ? 0.12 : 0.015);
  ...
  return (
    <group ref={groupRef} position={[4.5, 0, 0]}>
      <instancedMesh ref={meshRef} args={[null, null, 64]}>
        ...
        <meshStandardMaterial color="#00E5FF" metalness={1} roughness={0} emissive={isAssembled ? "#00E5FF" : "#000"} emissiveIntensity={0.5} />
      </instancedMesh>
...
const CompCard = ({ title, icon, skills }) => {
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className="bg-[#0A0A12]/40 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/5 hover:border-[#00E5FF]/40 transition-all duration-700 shadow-2xl flex flex-col h-[420px]">
      ...
      <div className="space-y-10 mt-auto">
        {skills.map((s, i) => (
          <div key={i}>
            <div className="flex justify-between text-[11px] font-bold text-gray-500 uppercase mb-3 transition-colors" style={{ color: hover ? 'white' : '' }}><span>{s.n}</span><span style={{ color: hover ? '#00E5FF' : '' }}>{s.v}</span></div>
...
export default function DesktopView() {
  ...
  return (
    <div className="min-h-screen bg-[#010102] text-gray-200 overflow-x-hidden selection:bg-[#00E5FF] selection:text-black">
      ...
      <div className="relative z-10 w-full flex flex-col px-24">
        <nav className="fixed top-0 left-0 w-full z-[100] bg-[#010102]/70 backdrop-blur-3xl border-b border-white/5 h-24 flex items-center justify-between px-24">
          ...
          <div className="flex gap-12 relative">
            {["Journey", "Stacks", "Builds", "Offline", "Connect"].map(t => (
              <a key={t} href={`#${t.toLowerCase()}`} onClick={() => setTab(t)} className="relative py-2 text-[10px] font-black uppercase text-gray-500 hover:text-white transition-colors tracking-widest">
                {t}
                {tab === t && <motion.div layoutId="nav-slider" className="absolute bottom-0 left-0 h-[2px] w-full bg-gradient-to-r from-[#00E5FF] to-[#FF8C00] shadow-[0_0_15px_#00E5FF]" />}
              </a>
            ))}
          </div>
        </nav>

        <section className="min-h-screen flex items-center pt-20" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl">
            <h1 className="text-[9.5rem] font-black text-white mb-4 tracking-tighter leading-[0.85]">Shivang <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00E5FF] via-white to-[#FF8C00]">Ayar.</span></h1>
            <p className="text-2xl text-gray-400 mt-6 mb-12 font-light max-w-2xl border-l-2 border-[#FF8C00] pl-8 leading-relaxed">Designing high-performance full-stack architectures and resilient digital systems.</p>
            <div className="flex gap-6"><a href="#builds" className="bg-white text-black px-12 py-5 text-[10px] font-black tracking-widest uppercase hover:bg-[#00E5FF] hover:text-white transition-all">Execute Builds</a><a href="/resume.pdf" target="_blank" className="border border-white/10 text-white px-12 py-5 text-[10px] font-black tracking-widest uppercase hover:bg-white hover:text-black transition-all">Resume ↓</a></div>
          </motion.div>
        </section>

        <section id="journey" className="py-40 relative">
          <div className="grid grid-cols-12 gap-16">
            <div className="col-span-5 sticky top-40"><h2 className="text-6xl font-black text-white mb-10 tracking-tighter uppercase leading-none">The <br/><span className="text-[#00E5FF]">Timeline.</span></h2><p className="text-gray-400 text-xl font-light leading-relaxed">Mastering software architecture through engineering precision.</p></div>
            <div className="col-span-7 space-y-12 border-l-2 border-[#00E5FF]/20 ml-4 relative">
        ...
        <section id="stacks" className="py-20 w-full"><h2 className="text-8xl font-black text-white mb-24 tracking-tighter text-center uppercase">Technical <span className="text-[#00E5FF]">Arsenal.</span></h2><div className="grid grid-cols-3 gap-10">
          <CompCard title="Languages" icon="💻" skills={[{n:"Java",v:"90%"},{n:"Python",v:"85%"},{n:"JS",v:"85%"},{n:"C#",v:"70%"}]} />
          <CompCard title="Frontend" icon="🖥️" skills={[{n:"React.js",v:"90%"},{n:"HTML5/CSS3",v:"95%"},{n:"Tailwind",v:"85%"},{n:"Next.js",v:"80%"}]} />
          <CompCard title="Backend" icon="⚙️" skills={[{n:"Node.js",v:"85%"},{n:"Express.js",v:"85%"},{n:"REST APIs",v:"90%"},{n:"WebSockets",v:"75%"}]} />
          <CompCard title="Databases" icon="🗄️" skills={[{n:"MongoDB",v:"85%"},{n:"MySQL",v:"90%"},{n:"PostgreSQL",v:"75%"},{n:"NoSQL",v:"85%"}]} />
          <CompCard title="Analytics" icon="📊" skills={[{n:"Power BI",v:"90%"},{n:"DAX",v:"85%"},{n:"Star Schema",v:"85%"},{n:"ETL",v:"80%"}]} />
          <CompCard title="DevOps" icon="☁️" skills={[{n:"GitHub",v:"95%"},{n:"AWS",v:"80%"},{n:"Docker",v:"75%"},{n:"CI/CD",v:"85%"}]} />
        </div></section>

        <section id="builds" className="py-40">
          ...
          <div className="grid grid-cols-3 gap-10">{projects.map((p, i) => (
            <motion.div key={i} whileHover={{ y: -15 }} className="bg-[#050507]/40 backdrop-blur-xl p-12 rounded-[3.5rem] border border-white/5 relative overflow-hidden h-[540px] flex flex-col group cursor-pointer shadow-2xl">
