    ...
    <motion.div exit={{ opacity: 0 }} className="fixed inset-0 z-[1000] bg-[#010102] flex flex-col items-center justify-center p-8 text-center">
      <div className="w-full max-w-xs space-y-10">
        <div className="space-y-2">
          <h2 className="text-[#00E5FF] font-mono text-[10px] tracking-[0.4em] animate-pulse uppercase">Security Protocol Active</h2>
          <div className="h-[2px] w-full bg-white/5 rounded-full overflow-hidden">
            <motion.div className="h-full bg-gradient-to-r from-[#00E5FF] to-[#FF8C00]" style={{ width: `${progress}%` }} />
          </div>
        </div>
        <p className="text-white font-black text-xs tracking-widest uppercase h-10 flex items-center justify-center leading-relaxed">{status}</p>
        <div className="text-[8px] font-mono text-gray-800 uppercase tracking-widest">User_Auth: Pending | encrypted_link: true</div>
      </div>
    </motion.div>
  );
};
...
function MechanicalCore({ scrollY }) {
  ...
  const isAssembled = scrollY < 150; // Cube snaps at the very top, shatters as you scroll down
  ...
  useFrame((state, delta) => {
    ...
    cubeData.forEach((data, i) => {
      ...
      currentP[i].lerp(tP, isAssembled ? 0.12 : 0.015);
      currentR[i].slerp(tR, isAssembled ? 0.12 : 0.015);
  ...
  return (
    <group ref={groupRef}>
      <instancedMesh ref={meshRef} args={[null, null, 64]}>
        ...
        <meshStandardMaterial color="#00E5FF" metalness={1} roughness={0.1} emissive="#00E5FF" emissiveIntensity={isAssembled ? 0.8 : 0.1} />
      </instancedMesh>
...
const CompCard = ({ title, icon, skills }) => {
  ...
  return (
    <div ref={ref} className="bg-[#0A0A12]/60 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl flex flex-col h-[380px]">
      <div className="flex justify-between items-center mb-10 text-white"><h3 className="text-lg font-black uppercase tracking-widest">{title}</h3><span className="text-2xl">{icon}</span></div>
      <div className="space-y-8 mt-auto">
        {skills.map((s, i) => (
          <div key={i}>
            <div className="flex justify-between text-[10px] font-bold text-gray-500 uppercase mb-3 transition-colors" style={{ color: active ? 'white' : '' }}><span>{s.n}</span><span style={{ color: active ? '#00E5FF' : '' }}>{s.v}</span></div>
            <div className="flex gap-1.5 h-[4px] overflow-hidden bg-white/2 rounded-full">
              {[...Array(10)].map((_, idx) => (
                <motion.div key={idx} initial={{ backgroundColor: "#14141B" }} animate={active ? { backgroundColor: idx * 10 < parseInt(s.v) ? "#00E5FF" : "#14141B" } : {}} transition={{ delay: i * 0.1 + idx * 0.04 }} className="flex-1 rounded-sm" style={{ backgroundImage: active && idx * 10 < parseInt(s.v) ? 'linear-gradient(to right, #00E5FF, #FF8C00)' : 'none' }} />
              ))}
            </div>
          </div>
        ))}
...
export default function MobileView() {
  ...
  return (
    <div className="min-h-screen bg-[#010102] text-gray-200 overflow-x-hidden selection:bg-[#00E5FF]">
      ...
      {/* 3D BACKGROUND */}
      <div className="fixed inset-0 z-0 pointer-events-none"><Canvas dpr={[1, 2]}><PerspectiveCamera makeDefault position={[0, 0, 30]} fov={75} /><ambientLight intensity={0.5} /><Environment preset="night" /><MechanicalCore scrollY={scrollY} /><ContactShadows position={[0, -12, 0]} opacity={0.4} scale={40} blur={3} color="#00E5FF" /></Canvas></div>
      ...
      <div className="relative z-30 w-full flex flex-col px-6">
        ...
        <section id="journey" className="py-32 space-y-12 border-l-2 border-[#00E5FF]/20 ml-2">
          {[{y:"2024 - PRES", t:"Algonquin College", d:"Advanced Diploma in Computer Programming. Focused on Enterprise Microservices."}, {y:"2021 - 2023", t:"FIC | BC", d:"Computer Science Pathway. Deep-dive into Big O efficiency and OOP logic."}].map((item, idx) => (
            <div key={idx} className="relative pl-10"><div className={`absolute -left-[11px] top-2 w-5 h-5 rounded-full shadow-[0_0_15px_#00E5FF] ${idx === 0 ? 'bg-[#00E5FF]' : 'bg-purple-500'}`} /><div className="bg-[#0A0A12]/60 p-8 rounded-[3rem] border border-white/5 shadow-2xl"><span className="text-[10px] font-black text-gray-500 tracking-[0.3em]">{item.y}</span><h3 className="text-2xl font-black text-white mt-4 tracking-tight">{item.t}</h3><p className="text-gray-400 text-lg mt-4 font-light leading-relaxed">{item.d}</p></div></div>
          ))}
        </section>
        ...
        <section id="builds" className="py-40 space-y-10">
          {[ {title:"Microservices", d:"Scalable ecosystem using Node.js and Docker. Integrated Stripe API and JWT auth."}, {title:"Watchlist App", d:"Full-stack MERN app via RESTful APIs and NoSQL architecture."}].map((p, i) => (
          <div key={i} className="bg-[#0A0A12]/60 p-10 rounded-[3rem] border border-white/5 shadow-2xl relative overflow-hidden"><div className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-[#00E5FF]/20 m-6" /><h3 className="text-3xl font-black text-white mb-6 uppercase tracking-tighter">{p.title}</h3><p className="text-gray-400 text-lg mb-8 leading-relaxed font-light">{p.d}</p></div>
        ))}</section>

        {/* OFFLINE PROTOCOL */}
        <section id="offline" className="py-32 space-y-8 text-center"><h2 className="text-5xl font-black text-white mb-16 tracking-tighter uppercase italic">Offline <span className="text-[#FF8C00]">Protocol.</span></h2>
          {[ {i:"🏋️‍♂️",t:"Discipline",d:"6-day compound split focus."}, {i:"🎮",t:"Logic",d:"Hardware tuning & shooters."}, {i:"🌍",t:"Equilibrium",d:"Hiking to reset the buffer."}].map((h,x)=>(
          <motion.div key={x} initial={{opacity:0, y:30}} whileInView={{opacity:1, y:0}} transition={{duration:0.6}} viewport={{once:true}} className="bg-[#0A0A12]/60 p-12 rounded-[3.5rem] border border-white/5 shadow-2xl"><div className="text-7xl mb-10">{h.i}</div><h3 className="text-2xl font-black text-white uppercase tracking-widest">{h.t}</h3><p className="text-gray-500 text-lg mt-6 leading-relaxed font-light">{h.d}</p></motion.div>
        ))}</section>

        {/* FOOTER */}
        <footer id="connect" className="py-40 flex items-center justify-center text-center"><div className="w-full bg-[#020203] border-2 border-[#00E5FF]/40 p-12 rounded-[5rem] shadow-2xl relative overflow-hidden"><div className="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-[#00E5FF] to-[#FF8C00]" /><h2 className="text-6xl font-black text-white mb-10 tracking-tighter uppercase leading-none">Terminal <br/><span className="text-[#00E5FF]">Ready.</span></h2><div className="flex flex-col gap-6 mt-10"><a href="https://github.com/ayarshivang27" className="bg-white/5 border border-white/10 px-10 py-6 rounded-3xl text-white font-black tracking-widest uppercase">GitHub</a><a href="https://linkedin.com/in/shivangayar/" className="bg-white/5 border border-white/10 px-10 py-6 rounded-3xl text-white font-black tracking-widest uppercase">LinkedIn</a><a href="mailto:ayarshivang27@gmail.com" className="bg-white/5 border border-white/10 px-10 py-6 rounded-3xl text-white font-black tracking-widest uppercase">Email</a></div><p className="mt-40 text-[10px] font-black tracking-[1.5em] text-gray-800 uppercase leading-relaxed">© 2026 SHIVANG AYAR.<br/>ARCHITECTED WITH INTENT.</p></div></footer>
      </div>
    </div>
  );
}