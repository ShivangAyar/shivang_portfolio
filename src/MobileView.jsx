...
// --- MECHANICAL SNAP CORE (Exact Desktop logic) ---
...
const ReactiveFooter = () => {
  ...
  return (
    <footer id="connect" className="py-40 flex items-center justify-center relative overflow-hidden px-6">
      ...
      <div className="w-full bg-[#020203] border-2 border-[#00E5FF]/30 p-12 rounded-[5rem] shadow-[0_0_80px_rgba(0,229,255,0.08)] relative z-10 overflow-hidden">
        {/* CRT Scanline Effect */}
        ...
        <motion.h2 
          ...
          className="text-6xl font-black text-white mb-8 tracking-tighter uppercase leading-none text-center"
        >
        ...
        <div className="bg-black/50 border border-white/5 p-5 rounded-2xl mb-12 font-mono text-[9px] text-[#00E5FF]/60 text-left min-h-[90px] backdrop-blur-md">
          {logs.map((line, idx) => (
            <div key={idx} className="flex gap-2 mb-1">
              <span className="text-[#FF8C00]">#</span> {line}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#010102] text-gray-200 overflow-x-hidden selection:bg-[#00E5FF]">
      ...
      <div className="relative z-30 w-full flex flex-col">
        ...
        <AnimatePresence>{isMenuOpen && (<motion.div initial={{ opacity: 0, x: '100%' }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: '100%' }} className="fixed inset-0 z-[105] bg-[#010102] flex flex-col items-center justify-center gap-10">{['About', 'Skills', 'Builds', 'Connect'].map(t => <a key={t} href={`#${t.toLowerCase()}`} onClick={() => setIsMenuOpen(false)} className="text-4xl font-black uppercase tracking-widest">{t}</a>)}</motion.div>)}</AnimatePresence>
        ...
        <section id="about" className="px-6 py-32 flex flex-col gap-10"><h2 className="text-5xl font-black text-white mb-8 tracking-tighter uppercase">About <span className="text-[#00E5FF]">Me.</span></h2><p className="text-gray-400 text-lg font-light leading-relaxed">Born and raised in Zambia, now operating in Ottawa. My approach to engineering is purely objective: Build, Optimize, and Master.</p></section>
        <section className="px-6 py-10 space-y-12 border-l-2 border-[#00E5FF]/20 ml-2">
          {[{y:"2024 - PRES", t:"Algonquin College", d:"Advanced Diploma in Computer Programming. Enterprise focus."}, {y:"2021 - 2023", t:"FIC | BC", d:"Computer Science Pathway. Specialized expertise in algorithm design."}].map((item, idx) => (
            <div key={idx} className="relative pl-10"><div className={`absolute -left-[11px] top-2 w-5 h-5 rounded-full shadow-[0_0_15px_#00E5FF] ${idx === 0 ? 'bg-[#00E5FF]' : 'bg-purple-500'}`} /><div className="bg-[#050507]/40 p-8 rounded-[3rem] border border-white/5 shadow-2xl"><span className="text-[10px] font-black text-gray-500 tracking-[0.3em]">{item.y}</span><h3 className="text-xl font-black text-white mt-4 tracking-tight">{item.t}</h3><p className="text-gray-400 text-sm mt-4 font-light leading-relaxed">{item.d}</p></div></div>
          ))}
        </section>
        <section id="skills" className="px-6 py-32 space-y-8"><h2 className="text-6xl font-black text-white mb-16 tracking-tighter uppercase text-center">Core Stacks.</h2><CompCard title="Programming" icon="💻" skills={[{n:"Java",v:"90%"},{n:"Python",v:"85%"},{n:"JS",v:"85%"}]} /><CompCard title="Frontend" icon="🖥️" skills={[{n:"React",v:"90%"},{n:"HTML/CSS",v:"95%"},{n:"Tailwind",v:"85%"}]} /><CompCard title="Backend" icon="⚙️" skills={[{n:"Node",v:"85%"},{n:"REST",v:"90%"},{n:"WS",v:"75%"}]} /></section>
        <ReactiveFooter />
      </div>
    </div>
  );
